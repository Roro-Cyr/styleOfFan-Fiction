...
def merge_files(csv_file_path, txt_file_path, output_file_path):
    ...
    with open(txt_file_path, mode='r') as txtfile:
        for line in txtfile:
            work_id, distanceToNarnia, distanceToWorm = line.strip().split()
            work_id = work_id.strip('"')  # Remove the surrounding double quotes
            txt_data[work_id] = {'distanceToNarnia': distanceToNarnia, 'distanceToWorm': distanceToWorm}
    ...
    for work_id, values in csv_data.items():
        if work_id in txt_data:
            merged_data.append({
                ...
                'kudos': values['kudos'],
                'hits': values['hits'],
                'distanceToNarnia': txt_data[work_id]['distanceToNarnia'],
                'distanceToWorm': txt_data[work_id]['distanceToWorm']
            })

    # Write the merged data to a new CSV file
    with open(output_file_path, mode='w', newline='') as csvfile:
        fieldnames = ['work_id', 'kudos', 'hits', 'distanceToNarnia', 'distanceToWorm']
